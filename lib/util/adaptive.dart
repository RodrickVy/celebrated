...
class Adaptive {
  ...
  T orientationBuilder<T>({required T Function() portrait, required T Function() landscape}) {
  ...
  Y adaptBuilder<Y>({required Y Function() phone, required Y Function() tablet, required Y Function() desktop}) {
    if (smallDesktop) {
      return desktop();
    } else if (smallTablet) {
      return tablet();
  ...
  String get adaptState {
   ...
   T adaptScreens<T>({required T small, required T big}) {
    return adapt(phone: small, tablet: Adaptive(context).tablet(small: small, large: big), desktop: big);
  }
}
